...
## 🎨 Polish items deferred (from design audit)

These are design improvements that were intentionally deferred — listed here in case you want to revisit:

- [ ] **Editorial eyebrow treatment** — apply the rule-line eyebrow format (`— Our Reach —` style currently on home Stats) more broadly across major H2s, OR keep it precious to a few spots only
- [ ] **Nav hierarchy** — visual grouping/separator between primary and secondary nav items (skipped because 7 items is OK; will matter more if Foundation/Providers/Hosts brings it to 9+)
- [ ] **Mobile audit** — explicit testing pass on mobile viewports (currently relying on `hidden md:flex` to gracefully degrade hero visuals; haven't done device-level testing)
- [ ] **Section boundary hairlines** — subtle orange/25 rules between major content sections (utility class added but not applied broadly)
- [ ] **Page-specific accent colors** — assigning each page a primary brand accent (e.g., About = blue, Partners = green) to reduce orange overuse — *you said keep current orange-heavy treatment, so this is unlikely to revisit*

---

## 📋 Suggested priority order

If you want to knock these out fastest-to-slowest impact:

| # | Item | Effort | Blocking |
|---|---|---|---|
| 1 | **Stats refresh (#4 + #5)** | 5 min code change | Just need numbers |
| 2 | **Closed-loop verbiage (#6)** | 5 min code change | Just need approved copy |
| 3 | **About history (#7)** | 30 min code change | Need copy from team |
| 4 | **Nav order confirmation (#3)** | 5 min code change | Need decision |
| 5 | **Partners → Providers + Hosts split (#2)** | 1-2 hour code change | Need definitions + content split decision |
| 6 | **Foundation page (#1)** | 2-4 hour code change | Need copy + donation flow decision |
| 7 | **Page heading "Collaboration" (#8)** | 2 min code change | Need decision |

Items 1, 2, 4, 7 are the quick wins — once you have numbers/decisions, each is a sub-15-minute change.
Items 5 and 6 are larger because they involve new pages with structure + content.

---

## Process for closing items out

When content/decisions land for any item:
1. Drop it in this file (or just message me with the answers)
2. I'll create a feature branch (`feat/foundation-page`, `feat/stats-update`, etc.)
3. Build, push, get a Vercel preview URL
4. You review on the preview
5. Merge to main → live

Single-item PRs keep the diff reviewable and the production rollouts low-risk.
